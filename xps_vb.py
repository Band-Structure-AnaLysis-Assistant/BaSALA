...
def calculate_shirley_bg(x, y, tol=1e-5, max_iters=50):
    ...
    # 2. 基準点の決定 (右端=始点, 左端=終点)
    I_start = y_sorted[0]  # Low BE side (Right)
    I_end = y_sorted[-1]   # High BE side (Left)
...
class XPS_VB_Edge_App(ctk.CTk):
    def __init__(self):
        ...
        # --- ウィンドウ設定 ---
        self.title("XPS Analysis Suite - v0.8 (Switchable Modes)")
        ...
        # --- メンバ変数（ツール用）---
        self.span = None        # マウス選択範囲 (SpanSelector)
        self.selection_mode = None # 現在どの入力欄を選択中か
    ...
    def _init_bandgap_tab(self):
        ...
        self.seg_bg_mode.set("Linear Fit") # 初期値

        # 1. Main Peak (共通項目: 常に表示)
        ...
        # --- Linear Mode用入力エリア ---
        self.frame_linear = ctk.CTkFrame(self.bg_tab_frame, fg_color="transparent")
        ...
        # --- Derivative Mode用入力エリア ---
        self.frame_deriv = ctk.CTkFrame(self.bg_tab_frame, fg_color="transparent")
        
        # Onset Region (1つの範囲でOK)
    ...
    def update_bg_ui(self, value):
        """モード切替時にUI部品を出し分ける"""
        if value == "Linear Fit":
            self.frame_deriv.pack_forget() # Derivを隠す
            self.frame_linear.pack(fill="x", pady=5) # Linearを出す
        else:
            self.frame_linear.pack_forget() # Linearを隠す
            self.frame_deriv.pack(fill="x", pady=5) # Derivを出す
    ...
    def _create_main_area(self):
        ...
        self.ax.invert_xaxis() # XPS Convention
    ...
    def apply_graph_settings(self):
        ...
        try:
            ...
            # 軸範囲 (set_xlimは min, max どちらが左でも自動判定するが、XPS用に明示的にセット)
    ...
    def activate_selector(self, mode):
        ...
        # モードごとの色分け
        colors = {
            'bg': 'blue', 'slope': 'red',             # VBM Analysis
            'bg_peak': 'green',                       # Band Gap: Peak
            'bg_base': 'blue', 'bg_slope': 'red',     # Band Gap: Linear
            'bg_deriv': 'orange'                      # Band Gap: Derivative
        }
    ...
    def calculate_bandgap(self):
        ...
        try:
            y_data = self.get_current_intensity()
            mode = self.bg_mode_var.get() # "Linear Fit" or "Derivative"
            
            # 1. Main Peak位置の特定 (共通処理)
            ...
            if mode == "Linear Fit":
                # === Method A: 直線交点法 (Linear Intersection) ===
                ...
                # === Method B: 2次微分法 (Second Derivative Method) ===
